...
@Named
@ViewScoped
public class RelatorioListagemDebitosBean implements Serializable {
    ...
    public void imprimir() {
        try {
            String where = "";
            if(vencidas){
                where = "\nAND MP.DT_VENCIMENTO < "+ Util.parseSqlDate(new Date());
            }
            servico.imprimirListagemDebitos(global.getUsuarioLogado().getOrgao().getIdOrgao(),inscricao,validade,
                    global.getExercicio().getAno(), global.getUsuarioLogado().getLogin(), where);
        } catch (Exception ex) {
